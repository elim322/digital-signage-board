    ...
    for ($i=0;$i<$count;$i++) {
		...
		if ($wpf_vars['expire'] < time()) {
			switch ($wpf_vars['service']) {
				...
				case "bug":
					$w1 = bug_get_weather($wpf_vars['BUG_BASE_URI'],$wpf_vars['apikey1'], $wpf_vars['location'],$wpf_vars['metric']);
					$weather1=bug_xml_parser($w1);
					$w2 = bug_get_weather($wpf_vars['BUG_FORC_URI'],$wpf_vars['apikey1'], $wpf_vars['location'],$wpf_vars['metric']);
				...
				case "google":
					...
					$wpf_vars['location'], substr($wpf_vars['wpf_language'],0,2));
					$weather=google_xml_parser(utf8_encode($w));
					break;
			}

			// overwrite visitor specific data
			if ($wpf_vars['visitorlocation']) {
