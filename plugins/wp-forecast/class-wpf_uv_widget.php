...
if ( !class_exists('wpf_uv_widget') ) {
	class wpf_uv_widget extends WP_Widget
	{
		...
		function __construct() {
			$widget_ops = array('classname' => 'wp_forecast_uv_widget',
		...
		function widget( $args, $instance ) {
			pdebug(1, "Start of function class wpf_uv_widget::widget ()");
			...
			pdebug(1, "End of function  class wpf_uv_widget::widget () ");
		}
		...
		function update( $new_instance, $old_instance ) {
			pdebug(1, "Start of wpf_uv_widget::update()");
			...
			pdebug(1, "End of wpf_uv_widget::update()");
		}
		...
		function form( $instance ) {
			pdebug(1, "Start of wpf_uv_widget::form()");
		...
		function show($wpfcid,$args,$wpfvars, $showgraph, $safeexposure) {
			...
			if ( $w['location'] == "" or $wpfvars['visitorlocation'] == 1)
			...
			if (trim($ouv_apikey)!="") {
				if ($ouv_uv)
					$out .= "<div>".__('Current UV-Index',"wp-forecast_".$wpf_language).": ".$w['openuv']['uv']."</div>\n";
				if ($ouv_uvmax)
					$out .= "<div>".__('Max. UV-Index',"wp-forecast_".$wpf_language).": ".$w['openuv']['uv_max']."</div>\n";
				if ($ouv_ozone)
					$out .= "<div>".__('Ozone',"wp-forecast_".$wpf_language).": ".$w['openuv']['ozone']." DU</div>\n";
				$out .= "<br/>";
				if ($ouv_safetime || $safeexposure) {
					$j=1;
					foreach ($w['openuv']['safe_exposure_time'] as $set) {
						if (trim($set) != "")
							$out .= "<div>".__('Safe Exposure Time for Skin Type',"wp-forecast_".$wpf_language)." $j: ".$set." Min.</div>\n";
						else
							$out .= "<div>".__('Safe Exposure Time for Skin Type',"wp-forecast_".$wpf_language)." $j: &infin; Min.</div>\n";
						$j++;
					}
					$out .= "<br/>";
				}
