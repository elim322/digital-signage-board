...
function wpf_sub_admin_form($wpfcid,$widgetcall) {
  ...
  if (isset($_POST['info_update'])) {
	...
	if (! isset($_POST["visitorlocation"])) $_POST["visitorlocation"]=0;
    if ($av['visitorlocation'] != $_POST["visitorlocation"] and (!$ismulti or isset($allowed["ue_visitorlocation"]))) {
		$av['visitorlocation']  = $_POST["visitorlocation"];
        $upflag=true;
    }
    ...
    if (!isset($_POST["csssprites"]))  $_POST["csssprites"] = "0";
    ...
    // validate options for OpenUV.io
    if ($av['ouv_apikey'] != $_POST["ouv_apikey"] and !$ismulti ) {
    	$av['ouv_apikey'] =  $_POST["ouv_apikey"];
    	$upflag=true;
    }
    
    if (! isset($_POST["ouv_uv"])) $_POST["ouv_uv"]=0;
    if ($av['ouv_uv'] != $_POST["ouv_uv"] and !$ismulti ) {
    	$av['ouv_uv'] =  $_POST["ouv_uv"];
    	if ($av['ouv_uv']=="") $av['ouv_uv']="0";
    	$upflag=true;
    }
    
    if (! isset($_POST["ouv_uvmax"])) $_POST["ouv_uvmax"]=0;
    if ($av['ouv_uvmax'] != $_POST["ouv_uvmax"] and !$ismulti ) {
    	$av['ouv_uvmax'] =  $_POST["ouv_uvmax"];
    	if ($av['ouv_uvmax']=="") $av['ouv_uvmax']="0";
    	$upflag=true;
    }
    
    if (! isset($_POST["ouv_ozone"])) $_POST["ouv_ozone"]=0;
    if ($av['ouv_ozone'] != $_POST["ouv_ozone"] and !$ismulti ) {
    	$av['ouv_ozone'] =  $_POST["ouv_ozone"];
    	if ($av['ouv_ozone']=="") $av['ouv_ozone']="0";
    	$upflag=true;
    }
    
    if (! isset($_POST["ouv_safetime"])) $_POST["ouv_safetime"]=0;
    if ($av['ouv_safetime'] != $_POST["ouv_safetime"] and !$ismulti ) {
    	$av['ouv_safetime'] =  $_POST["ouv_safetime"];
...
?>
	...
	<a href="<?php echo plugin_dir_url( __FILE__ ); ?>/wp-forecast-search.php?height=600&amp;width=800&amp;wpfcid=<?php echo $wpfcid;?>" class="thickbox" title="">
		...
		&nbsp;&nbsp;&nbsp;<input type="checkbox" name="visitorlocation" id="visitorlocation" value="1" <?php if ($av['visitorlocation']=="1") echo "checked=\"checked\""?> onchange="vlocfields_update();" /><b><?php echo __('Use visitors location',"wp-forecast_".$locale)?></b>
	 </p>
...
<b><?php echo __('Forecast',"wp-forecast_".$locale)?></b>
       ...
       <?php 
			...
			echo ':<input name="ouv_apikey" id="ouv_apikey" type="text" size="40" maxlength="80" value="' . $av['ouv_apikey']. '" />&nbsp;&nbsp;&nbsp;';
			
			echo '<input type="checkbox" name="ouv_uv" id="oav_uv" value="1"';
			if ($av['ouv_uv']=="1") echo "checked=\"checked\"";
			echo "/>";
			...
			if ($av['ouv_uvmax']=="1") echo "checked=\"checked\"";
			echo "/>";
			...
			if ($av['ouv_ozone']=="1") echo "checked=\"checked\"";
			echo "/>";
			...
			if ($av['ouv_safetime']=="1") echo "checked=\"checked\"";
			echo "/>";
